"""
Hebrew-only SFT wrapper around scripts/chat_sft.py.

The original scripts/chat_sft.py hard-codes its training mixture
(SmolTalk + identity + MMLU + GSM8K + SimpleSpelling + SpellingBee) at
lines 163-180, and the CLI flags --mmlu-epochs/--gsm8k-epochs only zero
two of those entries. SmolTalk and the two spelling tasks remain.

This file is a near-byte-clone of scripts/chat_sft.py. The ONLY diffs:
  - Removed imports of SmolTalk / SimpleSpelling / SpellingBee / MMLU / GSM8K
  - Replaced the train_tasks / val_tasks construction (lines 163-180 in
    the original) with Hebrew-only CustomJSON tasks built from the files
    produced by scripts_he/prepare_sft_data_he.py
  - --mmlu-epochs and --gsm8k-epochs flags kept (accepted but ignored)
    for CLI compatibility.

Run identically to the original:
    torchrun --standalone --nproc_per_node=8 -m scripts_he.chat_sft_he -- \
        --model-tag=he_fw2_d20 --device-batch-size=16 --chatcore-every=-1
"""
...
# -----------------------------------------------------------------------------
# CLI arguments  (identical to scripts/chat_sft.py)
parser = argparse.ArgumentParser(description="Supervised fine-tuning (SFT) the model — Hebrew-only")
...
parser.add_argument("--chatcore-every", type=int, default=-1, help="evaluate ChatCORE metric every N steps (-1 = disable; default disabled for Hebrew because ChatCORE benchmarks English)")
parser.add_argument("--chatcore-max-cat", type=int, default=-1, help="max problems per categorical task for ChatCORE")
parser.add_argument("--chatcore-max-sample", type=int, default=24, help="max problems per generative task for ChatCORE")
parser.add_argument("--mmlu-epochs", type=int, default=0, help="(IGNORED in Hebrew wrapper; kept for CLI compat)")
parser.add_argument("--gsm8k-epochs", type=int, default=0, help="(IGNORED in Hebrew wrapper; kept for CLI compat)")
...
# -----------------------------------------------------------------------------
# *** Hebrew-only SFT data mixture ***
# The ONLY block that differs from scripts/chat_sft.py.
sft_train_path = os.path.join(base_dir, "sft_data_he.train.jsonl")
sft_val_path   = os.path.join(base_dir, "sft_data_he.val.jsonl")
identity_path  = os.path.join(base_dir, "identity_conversations_he.jsonl")

for p in (sft_train_path, sft_val_path, identity_path):
    if not os.path.exists(p):
        raise FileNotFoundError(
            f"Missing Hebrew SFT file: {p}\n"
            "Run:  python -m scripts_he.prepare_sft_data_he"
        )

train_tasks = [
    CustomJSON(filepath=sft_train_path),
    CustomJSON(filepath=identity_path),
    CustomJSON(filepath=identity_path),  # 2 epochs of identity
]
train_dataset = TaskMixture(train_tasks)
print0(f"Hebrew training mixture: {len(train_dataset):,} rows (HeQ+ParaShoot+Dolly-HE+OpenHermes-HE + identity x2)")

val_dataset = TaskMixture([CustomJSON(filepath=sft_val_path)])
# -----------------------------------------------------------------------------
...
while True:
    ...
    # ChatCORE is disabled by default in the Hebrew wrapper because it benchmarks English tasks.
...
get_report().log(section="SFT (Hebrew)", data=[
    user_config,
